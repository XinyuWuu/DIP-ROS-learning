...
int main(int argc, char **argv)
{
	...
	int frameWidth = capture.get(CAP_PROP_FRAME_WIDTH) / 2; // 图片宽 stereo camera divided by 2 to use only one side
	...
	while (ros::ok())
	{
		...
		src = frame(cv::Rect(0, 0, frame.cols / 2, frame.rows)).clone();
		...
		cmd_red.linear.x = abs(aim_area_pro - area_pro) * 2 < 0.0001 ? 0 : ((aim_area_pro - area_pro) > 0 ? (aim_area_pro - area_pro) * 4 : (aim_area_pro - area_pro) * 2);
		//cmd_red.linear.x = area_pro < aim_area_pro /50.0 ? 0 : cmd_red.linear.x ;
		cmd_red.linear.y = 0;
		cmd_red.linear.z = 0;
		cmd_red.angular.x = 0;
		cmd_red.angular.y = 0;
		cmd_red.angular.z = abs(-(midpoint.x - frameWidth / 2.0) / 250.0) < 0.15 ? 0 : -(midpoint.x - frameWidth / 2.0) / 250.0;
		pub.publish(cmd_red);
		ros::spinOnce();
